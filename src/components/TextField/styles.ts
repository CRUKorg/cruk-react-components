...
export const ExtraRight = styled(Extra)<ExtraProps>`
  box-sizing: border-box;
  width: auto;
...
export const StyledInput = styled.input<StyledInputProps>`
  ...
  min-width: 3em;
  padding: ${({ theme }) =>
    `calc((var(--_min-height, 48px) - (var(--size-border-width, 1px) * 2) - ${theme.typography.lineHeight} ) / 2) var(--spacing-xs, 1rem)`};
